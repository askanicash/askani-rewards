...
function updatePoints() {
    ...
    if (text) text.innerText = points + " / 100 Points";
}
...
function claimDailyBonus() {
    ...
    if (lastBonus) {
        ...
        if (hours < 24) {
            ...
            alert("Come back after " + remain + " hours.");
            return;
        }
    }
...
function withdrawRequest() {
    ...
    history.unshift(method + " - " + account + " - Rs.50");
    localStorage.setItem("history", JSON.stringify(history));

    updatePoints();
    loadHistory();

    let adminNumber = "923001234567"; // replace with your WhatsApp
    let message =
        "Withdrawal Request\n\n" +
        "Method: " + method + "\n" +
        "Account: " + account + "\n" +
